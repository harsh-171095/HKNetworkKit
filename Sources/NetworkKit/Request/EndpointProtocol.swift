...
/// adopt NetworkKit in an existing app. Conform an `enum` to it and pass the
/// cases straight to `APIClient.send(_:body:as:)`; NetworkKit converts them into
...
struct BridgedEndpoint: Endpoint {
    ...
    // Auth is expected to be supplied via `headers`, so NetworkKit's own
